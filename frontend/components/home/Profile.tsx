...
const Profile = ({ item, isEven, userId, setProfiles }: any) => {
    const colors = ["#F0F8FF", "#FFFFFF"];
    const [liked, setLiked] = useState(false);
    const [selected, setSelcted] = useState(false);
    const handleLike = async (selectedUserId: string) => {
        try {
            setLiked(true);
            await axios.post(`${URL_LOCAL}/send-like`, {
            ...
            setTimeout(() => {
                setProfiles((prevProfiles: any) =>
                    prevProfiles.filter((profile: any) => profile._id !== selectedUserId)
                );
                setLiked(false);
            }, 200);
        } catch (error) {
    ...
    const handleLikeOther = async (selectedUserId: any) => {
        try {
            setSelcted(true);
            await axios.post(`${URL_LOCAL}/send-like`, {
                currentUserId: userId,
            ...
            <View style={{ padding: 12, backgroundColor: "#F0F8FF" }}>
                <ScrollView showsHorizontalScrollIndicator={false} horizontal>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 50 }}>
                        ...
                        {item?.profileImages?.slice(0, 1).map(({ item, index }: any) => (
                            <Image
                                style={{
                                    width: 280,
                                    height: 280,
